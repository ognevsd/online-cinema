import pytest

from functional.settings import test_settings

pytestmark = pytest.mark.asyncio(loop_scope="session")


@pytest.mark.parametrize(
    "query_data, expected_answer",
    [
        (
            {"page_number": 1, "page_size": 50},
            {
                "status": 200,
                "length": 26,
                "page_number": 1,
                "total_pages": 1,
            },
        ),
        (
            {"page_number": 2, "page_size": 10},
            {
                "status": 200,
                "length": 10,
                "page_number": 2,
                "total_pages": 3,
            },
        ),
    ],
)
async def test_all(
    generate_data_from_file,
    es_write_data,
    make_get_request,
    query_data,
    expected_answer,
):
    # 1. Генерируем данные для ES
    bulk_query = generate_data_from_file(
        test_settings.test_data_path / "all_genres.json", "genres"
    )
    # 2. Загружаем данные в ES
    await es_write_data(bulk_query, "genres", "genre_index.json")

    # 3. Запрашиваем данные из ES по API

    body, _, status = await make_get_request("genres/", query_data)

    # 4. Проверяем ответ
    assert status == expected_answer["status"]
    assert len(body.get("data")) == expected_answer["length"]
    assert body.get("page_number") == expected_answer["page_number"]
    assert body.get("total_pages") == expected_answer["total_pages"]


@pytest.mark.parametrize(
    "query_data, expected_answer",
    [
        (
            {"genre_id": "3d8d9bf5-0d90-4353-88ba-4ccc5d2c07ff"},
            {
                "status": 200,
                "genre_id": "3d8d9bf5-0d90-4353-88ba-4ccc5d2c07ff",
                "body_detail": None,
            },
        ),
        (
            {"genre_id": "3d8d9bf5-0d90-4353-88ba-4ccc5d2c07f5"},
            {
                "status": 404,
                "genre_id": None,
                "body_detail": "Genre not found",
            },
        ),
    ],
)
async def test_genre_id(
    generate_data_from_file,
    es_write_data,
    make_get_request,
    query_data,
    expected_answer,
):
    # 1. Генерируем данные для ES
    bulk_query = generate_data_from_file(
        test_settings.test_data_path / "all_genres.json", "genres"
    )

    # 2. Загружаем данные в ES
    await es_write_data(bulk_query, "genres", "genre_index.json")

    # 3. Запрашиваем данные из ES по API

    body, _, status = await make_get_request(
        f"genres/{query_data['genre_id']}", {}
    )

    # 4. Проверяем ответ
    assert status == expected_answer["status"]
    assert body.get("uuid") == expected_answer["genre_id"]
    assert body.get("detail") == expected_answer["body_detail"]
